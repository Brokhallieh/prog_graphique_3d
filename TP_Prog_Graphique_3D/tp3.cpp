...
class Viewer: public EZCOGL::GLViewer
{
	...
	EZCOGL::MeshRenderer::UP grid_rend;
	EZCOGL::Texture2D::SP tex;
	EZCOGL::GLVVec4 colorMap;
...
{
	colorMap = {{0.0,        0.6372549,  0.88823529, 1.0},
	...
	auto meshGrid = EZCOGL::Mesh::Grid(100, 100);

	tex = EZCOGL::Texture2D::create();
	tex->load(DATA_PATH + "/tp3/IcelandHeightmap.png", false);

	shaderPrg = EZCOGL::ShaderProgram::create({{GL_VERTEX_SHADER, EZCOGL::load_src(SHADERS_PATH + "/tp3.vs")}, {GL_FRAGMENT_SHADER, EZCOGL::load_src(SHADERS_PATH + "/tp3.fs")}}, "Texturing");
	
	// ***********************************
	// Geometry
	// ***********************************
    // GRID
	grid_rend = meshGrid->renderer(1, -1, 2, -1, -1); // initiate a cube mesh renderer with a VAO containing 5 VBOs to
...
void Viewer::draw_ogl()
{
	...
	// You must bind your shader program and the texture
	shaderPrg->bind();
	tex->bind(0);
	...
	grid_rend->draw(GL_TRIANGLES); // the draw function of the mesh renderer handle the corresponding VAO binding
